...
p_Fig4 <- plot_studycomp(d_ils_labs, filename_prefix = "Fig04_")
...
readr::write_excel_csv(d_assays_plot, file = here(paste0("manuscript/output/FigS03A_sourcedata.csv")))

d_assays_filt <- flag_outlier(d_assays, limit_iqr = limit_iqr)  |> filter(!Outlier_sp, !Outlier_mp)
d_assays_filt$SampleType <- forcats::fct_relevel(d_assays_filt$SampleType, c("DB","hTAG","YAA","SRM"))
p_fig2B <- plot_single_vs_multipoint(d_assays_filt, c(0.8, 0.22, 7.2, 2.6))
readr::write_excel_csv(d_assays_filt, file = here(paste0("manuscript/output/FigS03B_sourcedata.csv")))
...
readr::write_excel_csv(d_assays_plot, file = here(paste0("manuscript/output/Fig04A_sourcedata.csv")))

d_assays_filt <- flag_outlier(d_assays, limit_iqr = limit_iqr) |> filter(!Outlier_mp)
d_assays_filt$SampleType <- forcats::fct_relevel(d_assays_filt$SampleType, c("SRM","hTAG","DB","YAA")) 
p_Fig3B <- plot_all_refmaterials(d_assays_filt, xy_lim = c(0.7, 0.25, 9, 3))
readr::write_excel_csv(d_assays_filt, file = here(paste0("manuscript/output/Fig04B_sourcedata.csv")))
...
readr::write_excel_csv(get_fig4_stats(d_assays_plot), file = here(paste0("manuscript/output/Fig04A_sourcedata_statistics.csv")))
readr::write_excel_csv(get_fig4_stats(d_assays_filt), file = here(paste0("manuscript/output/Fig04B_sourcedata_statistics.csv")))
```
...
p_Fig4B <- plot_studycomp(d_ils_labs_filt, filename_prefix = "FigS05")

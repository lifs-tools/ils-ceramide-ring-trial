...
# p2 <- ggplot(d_assays_filt  |> arrange(SampleType) , aes(x = C_SinglePoint_mean, y = C_Adj_mean, color = SampleType, fill = SampleType)) +
p2 <-
  ggplot(
    d_assays_filt  |> arrange(SampleType) ,
    aes(
  ...
  facetted_pos_scales(
    y = list(
      ceramideName == "Cer 18:1;O2/16:0" ~ scale_y_continuous(limits = c(0, 0.6)),
      ceramideName == "Cer 18:1;O2/18:0" ~ scale_y_continuous(limits = c(0, 0.125)),
      ceramideName == "Cer 18:1;O2/24:0" ~ scale_y_continuous(limits = c(0, 5.5)),
      ceramideName == "Cer 18:1;O2/24:1" ~ scale_y_continuous(limits = c(0, 1.8))
    ),
    x = list(
      ceramideName == "Cer 18:1;O2/16:0" ~ scale_x_continuous(limits = c(0, 0.6)),
      ceramideName == "Cer 18:1;O2/18:0" ~ scale_x_continuous(limits = c(0, 0.125)),
      ceramideName == "Cer 18:1;O2/24:0" ~ scale_x_continuous(limits = c(0, 5.5)),
      ceramideName == "Cer 18:1;O2/24:1" ~ scale_x_continuous(limits = c(0, 1.8))
    )
  ) + 
  scale_color_manual(
    values = c(
  ...
  ggsave(plot = p2, filename = here("manuscript/output/Fig02_Comparison_SingleVsMultiCalib.png"), device = "png",
...
d_ils_orig <- read_csv(here("manuscript/data/Cer-ILS-Ringtrial_FullDataset_SLING_V1.csv"))
...
fs::file_copy(path = here("output/CalibrationLinesAvgMedRangeLog10-with-NISTSRM-AvgRatio.png"), new_path = here("manuscript/output/FigS01_CalibrationLinesAvgMedRangeLog10-with-NISTSRM-AvgRatio.png"))
```
...
```
# 11. Figure S3: Normalization with different ISTDs (single-point calibration)
...
```{r figS3-plot, fig.height=8, fig.width=6, out.width="100%"}
...
ggsave(plot = p_figS4, filename = here("manuscript/output/FigS03_DifferentISTDs-SP.png"), device = "png",
dpi = 600, width =80, height =65, units = "mm")
...
# 12. Figure S4: Normalization to NIST SRM 1950 for hTAG
...
```{r figS04, fig.height=8, fig.width=12, out.width="100%"}
inputs <- list(
  list(sample_type="hTAG", plot_file_prefix="FigS04"),
  list(sample_type="DB", plot_file_prefix="FigS05"),
  list(sample_type="YAA", plot_file_prefix="FigS06")
)
...
# 13. Figure S6: Normalization to NIST SRM 1950 for DB
...
```{r figS05, fig.height=8, fig.width=12, out.width="100%"}

labScatterPlots[[2]]$p_comb
```

# 13. Figure S6: Normalization to NIST SRM 1950 for YAA
...
```{r figS06, fig.height=8, fig.width=12, out.width="100%"}

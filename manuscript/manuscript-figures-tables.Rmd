...
plot_single_vs_multipoint <- function(data, xy_lim) {
  ...
  readr::write_excel_csv(d_assays_filt, file = here(paste0("manuscript/output/Fig02_sourcedata.csv")))
...
readr::write_excel_csv(d_assays_filt, file = here(paste0("manuscript/output/Fig03_sourcedata.csv")))
...
p_Fig3
  ...
  d_stat <- d_assays_filt |> 
    ...
    summarise(
      ...
      CV = SD/MEAN *100)  |> 
    ungroup() 
readr::write_excel_csv(d_stat, file = here(paste0("manuscript/output/Fig03_sourcedata_statistics.csv")))
...
readr::write_excel_csv(d_plot_1, file = here(paste0("manuscript/output/Fig04_sourcedata_points.csv")))
readr::write_excel_csv(d_sum_1, file = here(paste0("manuscript/output/Fig04_sourcedata_statistics.csv")))
...
plot_diffistd <- function(data, y_title, n_label_offset = 20, legend_x_offset = 0.11, n_label_size = 1.3, fig_text = "") {
...
p_fig5  <- plot_diffistd(d_plot_norm_istd_mp, "\U003BCmol/L (multi-point calibration)", n_label_offset = 20, fig_text = "Fig5")
...
d_plot_stats <- d_plot_norm_istd_mp |> 
  ...
  readr::write_excel_csv(d_plot_norm_istd_mp, file = here(paste0("manuscript/output/", "Fig05", "_sourcedata_points.csv")))
  readr::write_excel_csv(d_plot_stats, file = here(paste0("manuscript/output/", "Fig05", "_sourcedata_statistics.csv")))
...
p_fig6C_allistd_mp  <- plot_diffistd(
  ...
  n_label_offset = 25, 
  legend_x_offset = 0.963, n_label_size = 2) + ggtitle("Multi-point calibration (all data)") + xlab("")

p_fig6A_allistd_mp_filt  <- plot_diffistd(
  data = d_plot_allistd_mp_filt, 
  y_title = "\U003BCmol/L (multi-point calibration)", 
  n_label_offset = 20, 
  legend_x_offset = 0.963, n_label_size = 2) + ggtitle("Multi-point calibration (outliers removed)")
...
pS6ABCD <- p_fig6A_allistd_mp_filt /  p_figS6B_allistd_sp_filt / p_fig6C_allistd_mp / p_figS6D_allistd_sp_alldata  + patchwork::plot_annotation(tag_levels = c("A", "B", "C", "D"))
...
# 18. Suppl Figure S7: Normalization to NIST SRM 1950 for hTAG
...
# 19. Suppl Figure S8: Normalization to NIST SRM 1950 for DB
...
# 20. Suppl Figure S9: Normalization to NIST SRM 1950 for YAA
...
```{r figS9-plot, fig.height=8, fig.width=12, out.width="100%"}
labScatterPlots[[3]]$p_comb
```


# 21. Additional Tables (not part of the manuscript)

## 21a - Summary calibration lines raw intensities of labelled vs non-labelled ceramide species
...
## 21b - Comparison between low- and high-resolution instruments in terms of concentration and inter-lab %CV
...
## 21c - Comparison SOP vs OTHER protocol types of single-point calibration derived concenctrations

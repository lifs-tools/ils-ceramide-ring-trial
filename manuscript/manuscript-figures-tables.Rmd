...
# Panel A: Passing-Bablok regression scatter ------------------------------------
pb_line <- regression_results |>
  filter(method == "PaBa") |>
  left_join(
    d_method_comp |> group_by(ceramideName, Dataset) |>
      summarise(xmax = max(C_SinglePoint_mean), .groups = "drop"),
    by = c("ceramideName", "Dataset")
  ) |>
  rowwise() |>
  mutate(x = list(seq(0, xmax, length.out = 50)),
         y = list(intercept + slope * seq(0, xmax, length.out = 50))) |>
  ungroup() |>
  select(ceramideName, Dataset, x, y) |>
  tidyr::unnest(c(x, y))

pb_lab <- regression_results |>
  filter(method == "PaBa") |>
  mutate(lab = sprintf("PB slope %.2f [%.2f, %.2f]\nint %.3f [%.3f, %.3f]",
                       slope, slope_lci, slope_uci,
                       intercept, intercept_lci, intercept_uci))

p_S10A <- ggplot(d_method_comp, aes(C_SinglePoint_mean, C_Adj_mean)) +
  geom_abline(slope = 1, intercept = 0, color = "#6c9694",
              linewidth = .3, linetype = "longdash") +
  geom_point(aes(color = SampleType, fill = SampleType),
             shape = 21, stroke = .33, size = 1.1, alpha = .6) +
  geom_line(data = pb_line, aes(x, y), color = "black", linewidth = .5) +
  geom_text(data = pb_lab, aes(x = -Inf, y = Inf, label = lab),
            hjust = -0.04, vjust = 1.1, size = 1.9, lineheight = .9) +
  facet_grid(Dataset ~ ceramideName, scales = "free") +
  scale_color_manual(values = pal_col, breaks = c("SRM", "hTAG", "DB", "YAA")) +
  scale_fill_manual(values = pal_fill, breaks = c("SRM", "hTAG", "DB", "YAA")) +
  labs(x = "\U003BCmol/L (single-point calibration)",
       y = "\U003BCmol/L (multi-point calibration)", color = NULL, fill = NULL) +
  theme_classic(base_size = 9) +
...
# Panel B: Bland-Altman percent differences (per-facet bias and LoA) -------------
...
# Panel C: concentration dependence with lowest calibration level ----------------
lowest_cal <- tibble(
...
ggsave(plot = p_S10A, filename = here("manuscript/output/FigS10A_Regression_PassingBablok.png"),
       device = "png", dpi = 600, width = 200, height = 115, units = "mm")
ggsave(plot = p_S10B, filename = here("manuscript/output/FigS10B_BlandAltman_PercentDifferences.png"),
       device = "png", dpi = 600, width = 200, height = 115, units = "mm")
ggsave(plot = p_S10C, filename = here("manuscript/output/FigS10C_ConcentrationDependence.png"),
       device = "png", dpi = 600, width = 200, height = 70, units = "mm")

p_FigS10 <- p_S10A / p_S10B / p_S10C +
  patchwork::plot_layout(heights = c(1, 1, 0.6)) +
  patchwork::plot_annotation(tag_levels = "A")
ggsave(plot = p_FigS10, filename = here("manuscript/output/FigS10_MethodComparison_Combined.png"),
       device = "png", dpi = 600, width = 200, height = 300, units = "mm")

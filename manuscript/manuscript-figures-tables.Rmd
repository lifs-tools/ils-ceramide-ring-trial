...
d_assays |> select(LabId, LabNo, LabNum) |> distinct() |>  write_csv(file = here("manuscript/output/Ids.csv")
)
```
...
plot_diffistd <- function(data, y_title) {
  
  data$ISTD <- forcats::fct_recode(data$ISTD, "AuthISTD" = "C_Adj_mean", "C16_ISTD" = "C_Adj_C16_mean" , C24_ISTD = "C_Adj_C24_mean" )
  data$ISTD <- forcats::fct_relevel(data$ISTD, c("AuthISTD", "C16_ISTD", "C24_ISTD"))
...
p_fig5  <- plot_diffistd(d_plot, "\U003BCmol/L (multi-point calibration)")
...
d_plot <- flag_outlier(d_assays, limit_iqr = limit_iqr) |> filter(!(Outlier_mp & SampleType != "SRM"))
d_plot <- flag_outlier(d_assays, limit_iqr = limit_iqr)


p_Fig6 <- plot_comparison_norm(d_assays = d_plot,variable = "C_Adj",sample_type = "DB",normalisation_sample = "SRM",save_plot = TRUE,filename_prefix = "Fig06_", pos = pos) + 
...
d_stats <- d_assays_filt |> 
  group_by(ceramideName, SampleType) |> 
  nest() |> 
    mutate(res = purrr::map(data, \(x) broom::tidy(t.test(C_SinglePoint_mean ~ MassAnalyzerResolution, paired = FALSE, var.equal = FALSE, alternative = "two.sided", data = x))
    )) |>
  unnest(res) |> 
  ungroup() |> 
  select(SampleType, ceramideName, p.value)
...
d_stats <- d_assays_filt |> 
  group_by(ceramideName, SampleType) |> 
  nest() |> 
    mutate(res = purrr::map(data, \(x) broom::tidy(t.test(C_SinglePoint_mean ~ Protocol, paired = FALSE, var.equal = FALSE, alternative = "two.sided", data = x))
    )) |>
  unnest(res) |> 
  ungroup() |> 
  select(SampleType, ceramideName, p.value)
...
# 14. Figure S6: Normalization with different ISTDs

Authentic vs class-specific internal standard normalization and its effects on bias and variability of concentrations from multi-point calibration of the full (A) and outlier-filtered (B) dataset.  The horizontal bars correspond to the mean and ±1 SD of all points per group. Data originating from the same laboratory are connected by dashed lines. Dataset 15 was excluded for Cer 18:1;O2/16:0 as it had 15x higher concentrations than the second highest point for C24:0 d7. SOP (red) and OTHER (blue) refer to the Standard vs Other methods, respectively. Outlier in the authentic internal standard groups have been removed (see Figure 1 and text).
...
p_fig5A  <- plot_diffistd(d_plot, "\U003BCmol/L (multi-point calibration)")

pS6 <- p_fig5A / p_fig5 + patchwork::plot_annotation(tag_levels = c("A", "B"))

ggsave(plot = pS6, filename = here("manuscript/output/FigS06_DifferentISTDs-All-FiltDataset.png"), device = "png",
dpi = 600, width =80, height =130, units = "mm")
...
# 15. Figure S7: Normalization with different ISTDs (single-point calibration)
...
d_raw <- bind_rows(d_raw_OTHER, d_raw_sop) |> 
  ...
  separate(LabId, into = c("LabNo", "MethodNo"), convert = FALSE, remove = FALSE, sep = "[abc]", fill = "right") |> 
  mutate(
    MethodNo = str_extract(LabId, "[abc]"),
    MethodNo = if_else(is.na(MethodNo), "a", MethodNo),
    Protocol = if_else(Protocol == "Standard", "SOP", "OTHER"), 
...
d_calc_plot <- d_calc |> 
  select(LabId, LabNum,ceramideName, Protocol, MethodNo, AuthISTD, C16_ISTD, C18_ISTD,C24_ISTD) |> 
  pivot_longer(names_to = "ISTD", values_to = "Concentration", cols = AuthISTD:C24_ISTD) |> 
...
d_plot <- d_calc_plot |> filter(str_detect(ceramideName, "16|24\\:0"), ISTD != "C18_ISTD") 


p_figS4  <- plot_diffistd(d_plot, "\U003BCmol/L (single-point calibration)")

ggsave(plot = p_figS4, filename = here("manuscript/output/FigS07_DifferentISTDs-SP.png"), device = "png",
dpi = 600, width =80, height =65, units = "mm")
...
d_plot_stats <- d_plot |> 
  ...
  dplyr::summarise(
    ...
    SD = sd(Concentration),
        CV = sd(Concentration)/mean(Concentration) *100)
write_csv(x = d_plot_stats, here("manuscript/output/FigS03_stats-sp.csv"))

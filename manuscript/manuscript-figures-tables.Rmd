...
# Save the used mapping of `LabId` and `LabNum.` 
#  `LabId` is the ID used for dataset submission and processing within the pipeline,
#  the letter (a and b) in the `LabId` indicate different datasets submitted by the 
#  same lab. 
#  `LabNum.` corresponds to renumbered lab id from `LabId`, which are used in the text, 
#  and in figures 1 and suppl. figures S02, S07-S09. Different datasets from the 
#  same lab are included under the same `LabNum`.
...
# Export data for Source Data file ------
d_srm_mean_filt_sp |> 
  select(SampleType, ceramideName, LabId, LabNum, Protocol, Protocol, 
         Conc_SP_mean = C_SinglePoint_mean, 
         Conc_SP_intralabCV = C_SinglePoint_intralabCV) |> 
  write_csv(here("manuscript/output/Table2_sourcedata_SP.csv"))

d_srm_mean_filt_mp |> 
  select(SampleType, ceramideName, LabId, LabNum, Protocol, Protocol, 
         Conc_MP_mean = C_Adj_mean , 
         Conc_MP_intralabCV = C_Adj_intralabCV) |> 
  write_csv(here("manuscript/output/Table2_TableS01-S02_sourcedata_MP.csv"))
```
...
tableS1_stats_all_datasets |> 
  ...
  write_excel_csv(file = here("manuscript/output/Suppl_TableS01_Concentrations-CVs_ALL-vs-Outlierfilt-datasets.csv"))
...
d_srm_mean |>   
  select(SampleType, ceramideName, LabId, LabNum, Protocol, Protocol, 
         Conc_MP_mean = C_Adj_mean) |> 
  write_csv(here("manuscript/output/Suppl_TableS01_sourcedata_ALL.csv"))

# Source for filtered data is identical to source data (multi-point calibrated) of Table 1 
...
write_excel_csv(
  x = tableS2_final,
  file = here("manuscript/output/Suppl_TableS02_Concentrations-CVs_SOP-vs_OTHER-MP.csv")
)

# Source data is identical to source data (multi-point calibrated) of Table 1 
```
...
## 22a - Calibration line raw intensities of labelled vs non-labelled ceramide species

Replicate injections per SRM sample were averaged. See important data file from the pipeline 
...
write_csv(x = d_caliblines_areas_mean, file = here("manuscript/output/Unpublished_Summary_bySample_byCalibrationLine_Areas-Light-Heavy.csv"))
...
Concentrations (in μmol/L) and inter-lab %CV of ceramide species measured by either low-resolution (QQQ) or high-resolution (QTOF/QE) MS instruments. The concentrations given in µmol/L were obtained using  multi-point calibration and filtered for outliers. The P values are from the comparison of individual values from the two instrument resolution categories (Welch’s two-sided t-test). N corresponds to the number of datasets after outlier removal.
...
## 22d - Concentrations derived based on individual calibration lines 

Concentrations (in μmol/L) calculated based on the individual calibration curves 1 or 2, and from single-point (internal standard-based) calibration.
Individual tables are created with all measurements, with mean and SD of replicate injections, and of samples per SRM type. 
Note: For datasets (LabId) 20b and 26b only one calibration curve was reported.    

```{r other-tables-srm_by-caliblines}
d_rawdata_byCalibLine_raw <- readr::read_csv( here("output/nistCal1Cal2AveragedConcentrationsLong.csv"), na = c("NA"))
d_ids <- readr::read_csv( here("manuscript/output/LabId-LabNum_mapping.csv"), na = c("NA"))


d_rawdata_byCalibLine_raw <- d_rawdata_byCalibLine_raw |>
  filter (Calibration == "C_CalibCurve_Avg") |> 
  mutate(LabId = str_replace(LabId, "^[1-9]$", paste0("0", LabId))) |> 
  left_join(d_ids, by = c("LabId")) |> 
  relocate(LabNum, .before = LabId)

# Average replicates per calibration line level
d_rawdata_byCalibLine_bySample <- d_rawdata_byCalibLine_raw |> 
  group_by(LabNum,	LabId,	SampleType,	Sample, ceramideName, Protocol, MassAnalyzerType) |> 
  summarise(
    Conc_singlepoint_rep_mean = mean(`C_SinglePoint_Calibration Line 1`, na.rm = TRUE),
    Conc_singlepoint_rep_sd = sd(`C_SinglePoint_Calibration Line 1`, na.rm = TRUE),
    Conc_Calib_1_rep_mean = mean(`C_Adj_Calibration Line 1`, na.rm = TRUE),
    Conc_Calib_1_rep_sd = sd(`C_Adj_Calibration Line 1`, na.rm = TRUE),
    Conc_Calib_2_rep_mean = mean(`C_Adj_Calibration Line 2`, na.rm = TRUE),
    Conc_Calib_2_red_sd = sd(`C_Adj_Calibration Line 2`, na.rm = TRUE)) |> 
  ungroup() 

d_rawdata_byCalibLine_bySRM <- d_rawdata_byCalibLine_bySample |> 
  ...
  summarise(
    ...
    Conc_Calib_1_mean = mean(Conc_Calib_1_rep_mean, na.rm = TRUE),
    Conc_Calib_1_sd = sd(Conc_Calib_1_rep_mean, na.rm = TRUE),
    Conc_Calib_2_mean = mean(Conc_Calib_2_rep_mean, na.rm = TRUE),
    Conc_Calib_2_sd = sd(Conc_Calib_2_rep_mean, na.rm = TRUE)) |> 
  ungroup()

write_csv(d_rawdata_byCalibLine_raw, here("manuscript/output/Unpublished_Summary_Concentrations_by_CalibLine_allData.csv"))
write_csv(d_rawdata_byCalibLine_bySample |> rename_with(~str_remove(., "_rep")), here("manuscript/output/Unpublished_Summary_Concentrations_by_CalibLine_bySample.csv"))
write_csv(d_rawdata_byCalibLine_bySRM, here("manuscript/output/Unpublished_Summary_Concentrations_by_CalibLine_bySRM.csv"))

```

## 22e Suppl Figure S10: Method comparison - Single- vs multi-point calibration

Comprehensive method comparison between single-point (SP) and multi-point (MP, calibration-curve) quantification, using all four ceramides in all four reference materials across the participating laboratories. Passing-Bablok regression is fitted per ceramide, pooling the four reference materials so that the between-material concentration differences provide the range needed to estimate a slope; the slope reflects proportional systematic error (proportional bias when the 95% CI excludes 1) and the intercept reflects constant systematic error (constant bias when the 95% CI excludes 0). Deming regression (assuming equal analytical error in both methods) is reported alongside as a sensitivity check. Both regression estimates are provided in the statistics file (FigS10_regression_PassingBablok_Deming_statistics.csv); ordinary least-squares regression is intentionally not used, as it assumes the SP axis is error-free. Panel A shows Bland-Altman percent differences ((MP - SP)/mean * 100) with per-ceramide bias (red solid) and limits of agreement (red dotted, mean ± 1.96 SD). Panel B shows the percent difference against mean concentration (log scale) with the lowest calibration level indicated, to assess whether the discrepancies are concentration-/LOQ-dependent. Panel A is shown for the full ("All data") and the outlier-filtered dataset. Panel B is shown for the full ("All data") dataset. Points are coloured by reference material: NIST SRM 1950 (SRM, blue), hypertriglyceridemic (hTAG, red), diabetic (DB, orange) and African American (YAA, light blue) of NIST RM 8231.

Passing-Bablok versus Deming regression — robustness and sensitivity. The two regressions rest on different assumptions and therefore behave differently on inter-laboratory data. Passing-Bablok is a non-parametric, rank-based estimator (its slope is the median of all pairwise slopes); it assumes neither a particular error distribution nor a known ratio of analytical error variances between the two methods, and it is robust to outliers, tolerating up to roughly 29% aberrant points before the estimate breaks down. Deming regression is a parametric errors-in-variables (least-squares) method that assumes approximately Gaussian, homoscedastic (constant-variance) errors in both methods with a known error-variance ratio (here assumed equal, λ = 1). When those assumptions hold, Deming is the more efficient — i.e. more sensitive — estimator and yields narrower confidence intervals; however, it is not robust, because outliers exert high leverage and because the constant-variance assumption is violated by the roughly constant-CV (proportional) error structure typical of these concentration data. In practice, therefore, Passing-Bablok is the more robust choice and Deming the more sensitive one only when its assumptions are met. Our data illustrate this trade-off directly: on the full dataset the Deming slope confidence intervals are on average 3.2-fold wider than the Passing-Bablok intervals (for example Cer 18:1;O2/18:0: Deming slope 1.72 [1.31, 2.13] versus Passing-Bablok 1.52 [1.41, 1.65]), and the Deming slope estimates shift more when outliers are removed (mean absolute change 0.064 versus 0.040 for Passing-Bablok). After outlier removal the two methods converge closely (mean confidence-interval-width ratio 1.3, slopes within ~0.05). We therefore report Passing-Bablok as the primary estimator, as it is robust to the outlier-prone and heteroscedastic structure of the inter-laboratory data, and Deming as a sensitivity check; the close agreement of the two after outlier filtering supports the reported proportional biases (notably the ~0.7 and ~1.5-1.6 slopes for Cer 18:1;O2/16:0 and Cer 18:1;O2/18:0).

```{r FigS10-plot, fig.height=10, fig.width=12, out.width="100%", message=FALSE, warning=FALSE}

library(mcr)

# Palettes consistent with Figure 2
pal_col  <- c(SRM = "#0571b0", hTAG = "#ca0020", YAA = "#92c5de", DB = "#f4a582")
pal_fill <- c(SRM = "#2c7bb6", hTAG = "#d7191c", YAA = "#abd9e9", DB = "#fdae61")
cer_levels <- c("Cer 18:1;O2/16:0", "Cer 18:1;O2/18:0",
                "Cer 18:1;O2/24:0", "Cer 18:1;O2/24:1")

# Prepare method-comparison data for the full and the outlier-filtered dataset -----
flagged <- flag_outlier(d_srm_mean, limit_iqr = limit_iqr)

make_mc <- function(df, label) {
  df |>
    select(SampleType, ceramideName, LabId, LabNum, Protocol, LC,
           C_SinglePoint_mean, C_Adj_mean) |>
    filter(!is.na(C_SinglePoint_mean), !is.na(C_Adj_mean)) |>
    mutate(
      diff = C_Adj_mean - C_SinglePoint_mean,
      mean_conc = (C_Adj_mean + C_SinglePoint_mean) / 2,
      percent_diff = diff / mean_conc * 100,
      Dataset = label
    )
}

d_method_comp <- bind_rows(
  make_mc(flagged, "All data"),
  make_mc(flagged |> filter(!Outlier_sp, !Outlier_mp), "Outlier-filtered")
) |>
  mutate(
    ceramideName = factor(ceramideName, levels = cer_levels),
    Dataset = factor(Dataset, levels = c("All data", "Outlier-filtered"))
  )

readr::write_excel_csv(d_method_comp,
  here("manuscript/output/FigS10_method-comparison_sourcedata.csv"))

# Passing-Bablok and Deming regression per ceramide (materials pooled) ------------
fit_reg <- function(df, method) {
  ci <- if (method == "PaBa") "analytical" else "jackknife"
  tryCatch({
    fit <- mcr::mcreg(x = df$C_SinglePoint_mean, y = df$C_Adj_mean,
                      method.reg = method, method.ci = ci, na.rm = TRUE)
    co <- mcr::getCoefficients(fit)
    tibble(
      method = method, n = nrow(df),
      intercept = co["Intercept", "EST"],
      intercept_lci = co["Intercept", "LCI"], intercept_uci = co["Intercept", "UCI"],
      slope = co["Slope", "EST"],
      slope_lci = co["Slope", "LCI"], slope_uci = co["Slope", "UCI"]
    )
  }, error = function(e) NULL)
}

regression_results <- d_method_comp |>
  group_by(ceramideName, Dataset) |>
  group_modify(~ bind_rows(fit_reg(.x, "PaBa"), fit_reg(.x, "Deming"))) |>
  ungroup() |>
  mutate(
    proportional_bias = !(slope_lci <= 1 & slope_uci >= 1),
    constant_bias     = !(intercept_lci <= 0 & intercept_uci >= 0)
  )

readr::write_excel_csv(regression_results,
  here("manuscript/output/FigS10_regression_PassingBablok_Deming_statistics.csv"))

# Bland-Altman statistics per ceramide (materials pooled) ------------------------
bland_altman_results <- d_method_comp |>
  group_by(ceramideName, Dataset) |>
  summarise(
    n = n(),
    bias = mean(percent_diff, na.rm = TRUE),
    sd = sd(percent_diff, na.rm = TRUE),
    loa_lower = bias - 1.96 * sd,
    loa_upper = bias + 1.96 * sd,
    bias_ci_lower = bias - qt(0.975, n - 1) * sd / sqrt(n),
    bias_ci_upper = bias + qt(0.975, n - 1) * sd / sqrt(n),
    .groups = "drop"
  )

readr::write_excel_csv(bland_altman_results,
  here("manuscript/output/FigS10_bland-altman_statistics.csv"))

# Panel A: Bland-Altman percent differences (per-facet bias and LoA) -------------
p_S10B <- ggplot(d_method_comp, aes(mean_conc, percent_diff)) +
  geom_hline(yintercept = 0, color = "grey50", linetype = "dashed", linewidth = .4) +
  geom_point(aes(color = SampleType, fill = SampleType),
             shape = 21, stroke = .33, size = 1.1, alpha = .6) +
  geom_hline(data = bland_altman_results, aes(yintercept = bias),
             color = "red", linewidth = .6) +
  geom_hline(data = bland_altman_results, aes(yintercept = loa_lower),
             color = "red", linetype = "dotted", linewidth = .45) +
  geom_hline(data = bland_altman_results, aes(yintercept = loa_upper),
             color = "red", linetype = "dotted", linewidth = .45) +
  facet_grid(Dataset ~ ceramideName, scales = "free_x") +
  scale_color_manual(values = pal_col, breaks = c("SRM", "hTAG", "DB", "YAA")) +
  scale_fill_manual(values = pal_fill, breaks = c("SRM", "hTAG", "DB", "YAA")) +
  labs(x = "Mean concentration (\U003BCmol/L)",
       y = "Percent difference (MP - SP) / mean * 100", color = NULL, fill = NULL) +
  theme_classic(base_size = 9) +
...
# Panel B: concentration dependence with lowest calibration level ----------------
lowest_cal <- tibble(
  ceramideName = factor(cer_levels, levels = cer_levels),
  xlc = c(0.008, 0.008, 0.08, 0.08)
)
p_S10C <- d_method_comp |>
  filter(Dataset == "All data") |>
  ggplot(aes(mean_conc, percent_diff)) +
  geom_hline(yintercept = 0, color = "grey50", linetype = "dashed", linewidth = .4) +
  geom_vline(data = lowest_cal, aes(xintercept = xlc),
             color = "purple", linetype = "dotdash", linewidth = .4) +
  geom_point(aes(color = SampleType, fill = SampleType),
             shape = 21, stroke = .33, size = 1.1, alpha = .6) +
  geom_smooth(method = "lm", formula = y ~ x, se = TRUE,
              color = "black", linewidth = .5) +
  facet_wrap(~ ceramideName, scales = "free_x", ncol = 4) +
  scale_x_log10() +
  scale_color_manual(values = pal_col, breaks = c("SRM", "hTAG", "DB", "YAA")) +
  scale_fill_manual(values = pal_fill, breaks = c("SRM", "hTAG", "DB", "YAA")) +
  labs(x = "Mean concentration (\U003BCmol/L, log10); dot-dash line = lowest calibration level",
       y = "Percent difference (MP - SP) / mean * 100", color = NULL, fill = NULL) +
  theme_classic(base_size = 9) +
  theme(axis.text = element_text(size = 7),
        axis.title = element_text(size = 8, face = "bold"),
...
ggsave(plot = p_S10B, filename = here("manuscript/output/FigS10A_BlandAltman_PercentDifferences.png"),
       device = "png", dpi = 600, width = 200, height = 115, units = "mm")
ggsave(plot = p_S10C, filename = here("manuscript/output/FigS10B_ConcentrationDependence.png"),
       device = "png", dpi = 600, width = 200, height = 70, units = "mm")

p_FigS10 <- p_S10B / p_S10C +
  patchwork::plot_layout(heights = c(1, 0.6)) +
  patchwork::plot_annotation(tag_levels = "A")
ggsave(plot = p_FigS10, filename = here("manuscript/output/FigS10_MethodComparison_Combined.png"),
       device = "png", dpi = 600, width = 200, height = 200, units = "mm")

p_FigS10

# Key findings for the manuscript text ------------------------------------------
...
print(as.data.frame(bland_altman_results |>
  mutate(across(where(is.numeric), ~round(., 2)))), row.names = FALSE)
```

Response-factor diagnostic — theoretical versus measured calibration response. To explain the origin of the SP-vs-MP discrepancy, the measured calibration response of each ceramide is compared with the theoretical response expected under an equimolar labelled/unlabelled response (response factor RF = 1). The measured calibration slope (mean calibrant area ratio regressed on concentration with 1/x² weighting) is divided by the theoretical slope (1/ISTD concentration). The measured response departs markedly from equimolar for the two discrepant species — Cer 18:1;O2/16:0 responds ~31% above and Cer 18:1;O2/18:0 ~36% below the equimolar expectation — whereas Cer 18:1;O2/24:0 and Cer 18:1;O2/24:1 are close to equimolar (−1% and −10%). This per-ceramide response-factor departure, consistent with over- or under-statement of the vendor calibrant concentrations for 16:0 and 18:0, is the mechanistic driver of the SP-vs-MP difference (the same factors are recovered from the ratio of mean single-point to mean multi-point concentrations — 1.43, 0.66, 1.06 and 0.98 — the small residual reflecting the non-zero calibration intercept). Because single-point calibration already assumes the theoretical (equimolar) response, multiplying the multi-point concentration by this response factor reproduces the single-point concentration exactly; that is, the correction collapses multi-point onto single-point calibration. The response factor is therefore reported here as a diagnostic of the calibration response and is not applied as a post-hoc concentration correction. A trueness-based correction is not attempted, because the equimolar assumption cannot be independently confirmed for all species — for Cer 18:1;O2/18:0 the single-point value lies further from the external reference consensus than the multi-point value (Suppl. Fig. S11).

```{r FigS10-response-factor, fig.height=3, fig.width=7, out.width="100%", message=FALSE, warning=FALSE}

# Response-factor diagnostic: theoretical (equimolar) vs measured calibration response.
if (!exists("cer_levels")) cer_levels <- c("Cer 18:1;O2/16:0", "Cer 18:1;O2/18:0",
                                           "Cer 18:1;O2/24:0", "Cer 18:1;O2/24:1")

d_calib <- readr::read_csv(
  here("output/CalibrationLinesAvgMedRangeLog10-with-NISTSRM-AvgRatio.csv"),
  show_col_types = FALSE) |>
  filter(ceramideName %in% cer_levels)

# Measured calibration slope (1/x^2-weighted, as in the pipeline) vs theoretical slope
response_factor <- d_calib |>
  group_by(ceramideName, ISTD_CONC) |>
  summarise(measured_slope = coef(lm(yavg ~ Concentration, weights = 1 / Concentration^2))[2],
            .groups = "drop") |>
  mutate(theoretical_slope = 1 / ISTD_CONC,
         response_factor = measured_slope / theoretical_slope,
         pct_dev_equimolar = 100 * (response_factor - 1),
         ceramideName = factor(ceramideName, levels = cer_levels)) |>
  arrange(ceramideName)

# Response factor as it manifests in the reported concentrations (mean SP / mean MP);
# multiplying MP by this factor reproduces SP (theoretical-response correction = SP).
rf_conc <- flag_outlier(d_srm_mean, limit_iqr = limit_iqr) |>
  filter(!Outlier_sp, !Outlier_mp, !is.na(C_SinglePoint_mean), !is.na(C_Adj_mean)) |>
  filter(ceramideName %in% cer_levels) |>
  group_by(ceramideName) |>
  summarise(SP = mean(C_SinglePoint_mean), MP = mean(C_Adj_mean), .groups = "drop") |>
  mutate(RF_conc_SPoverMP = SP / MP,
         MP_corrected_to_theoretical = MP * RF_conc_SPoverMP,
         ceramideName = factor(ceramideName, levels = cer_levels)) |>
  arrange(ceramideName)

response_factor_table <- response_factor |>
  select(ceramideName, theoretical_slope, measured_slope, response_factor, pct_dev_equimolar) |>
  left_join(rf_conc |> select(ceramideName, RF_conc_SPoverMP), by = "ceramideName")

readr::write_excel_csv(response_factor_table,
  here("manuscript/output/FigS10_response-factor_diagnostic.csv"))

p_FigS10rf <- response_factor |>
  ggplot(aes(ceramideName, response_factor)) +
  geom_hline(yintercept = 1, linetype = "dashed", color = "grey50", linewidth = .5) +
  geom_segment(aes(xend = ceramideName, y = 1, yend = response_factor),
               color = "grey60", linewidth = .4) +
  geom_point(size = 2.4, color = "#0571b0") +
  geom_text(aes(label = sprintf("%+.0f%%", pct_dev_equimolar)), vjust = -1, size = 2.6) +
  scale_y_continuous(limits = c(0, NA), expand = expansion(mult = c(0, 0.15))) +
  labs(x = NULL, y = "Response factor\n(measured / theoretical)",
       caption = "Dashed line = equimolar (theoretical) response, RF = 1") +
  theme_classic(base_size = 9) +
  theme(axis.text.x = element_text(size = 7),
        axis.title = element_text(size = 8, face = "bold"),
        plot.caption = element_text(size = 7, hjust = 0))

ggsave(plot = p_FigS10rf, filename = here("manuscript/output/FigS10_ResponseFactor_diagnostic.png"),
       device = "png", dpi = 600, width = 120, height = 70, units = "mm")

p_FigS10rf

cat("\n=== THEORETICAL vs MEASURED CALIBRATION RESPONSE (per ceramide) ===\n")
print(as.data.frame(response_factor_table |> mutate(across(where(is.numeric), ~round(., 3)))), row.names = FALSE)
cat("\n=== Theoretical-response correction reproduces single-point (SP) ===\n")
print(as.data.frame(rf_conc |> mutate(across(where(is.numeric), ~round(., 3)))), row.names = FALSE)
```

## 22f Suppl Figure S11: Trueness contextualization against external SRM 1950 references

Neither Passing-Bablok nor Bland-Altman can establish which calibration approach is more accurate, because both compare the two methods only to each other. The study data are derived from Torta et al. (Nat. Commun. 2024) and the outlier-filtered inter-lab consensus is not independent of the SP/MP measurements, so it cannot serve as a trueness anchor. Instead, the SP and MP inter-laboratory consensus values for NIST SRM 1950 are contextualized against independent external quantifications of the same material: Quehenberger et al. 2010, the Bowden et al. 2017 inter-laboratory study, and the multi-platform (including orthogonal NMR) characterization by Mandal et al. (Anal. Chem. 2025, 97, 667-675). These external references disagree with one another by more than the SP-vs-MP difference for 16:0, 24:0 and 24:1, indicating that absolute ceramide quantification is strongly method-dependent; the SP-vs-MP discrepancy for those species therefore lies within the between-method variability of the field. The exception is Cer 18:1;O2/18:0, where both SP and MP fall below every external reference, with MP closer to the external consensus. Red points are this study (ILS) with ±1 SEM; grey points are external references (Wishart with ±1 SD; Bowden individual laboratories). Only NIST SRM 1950 is shown, as external reference concentrations are not available for the NIST RM 8231 materials.

```{r FigS11-plot, fig.height=4, fig.width=12, out.width="100%", message=FALSE, warning=FALSE}

harmonise_cer <- function(x) str_replace(x, fixed("d18:1"), "18:1;O2")

d_ref_que <- read_csv(here("manuscript/data/Quehenberger_Ceramides_Conc.csv"),
                      show_col_types = FALSE) |>
  transmute(Source = "Quehenberger 2010", ceramideName = harmonise_cer(Species),
            Conc, Conc_sd = NA_real_)
d_ref_bow <- read_csv(here("manuscript/data/Bowden_InterlabStudy_Ceramides_Conc.csv"),
                      show_col_types = FALSE) |>
  transmute(Source = "Bowden 2017", ceramideName = harmonise_cer(Species),
            Conc, Conc_sd = NA_real_)
d_ref_wis <- read_csv(here("manuscript/data/Wishart2025_SRM1950_Ceramides_Conc.csv"),
                      show_col_types = FALSE) |>
  transmute(Source = "Wishart 2025", ceramideName = harmonise_cer(Species),
            Conc, Conc_sd)

# Our SRM 1950 consensus: SP filtered on Outlier_sp, MP on Outlier_mp (as in Table 2)
srm_sp <- flagged |>
  filter(SampleType == "SRM", !Outlier_sp, !is.na(C_SinglePoint_mean)) |>
  group_by(ceramideName) |>
  summarise(Source = "ILS SP", Conc = mean(C_SinglePoint_mean),
            Conc_sd = sd(C_SinglePoint_mean) / sqrt(n()), .groups = "drop")
srm_mp <- flagged |>
  filter(SampleType == "SRM", !Outlier_mp, !is.na(C_Adj_mean)) |>
  group_by(ceramideName) |>
  summarise(Source = "ILS MP", Conc = mean(C_Adj_mean),
            Conc_sd = sd(C_Adj_mean) / sqrt(n()), .groups = "drop")

d_trueness <- bind_rows(d_ref_que, d_ref_bow, d_ref_wis, srm_sp, srm_mp) |>
  filter(ceramideName %in% cer_levels) |>
  mutate(
    ceramideName = factor(ceramideName, levels = cer_levels),
    Source = factor(Source, levels = c("Quehenberger 2010", "Bowden 2017",
                                        "Wishart 2025", "ILS SP", "ILS MP")),
    Origin = if_else(Source %in% c("ILS SP", "ILS MP"),
                     "This study (ILS)", "External reference")
  )

readr::write_excel_csv(d_trueness,
  here("manuscript/output/FigS11_trueness_SRM1950_sourcedata.csv"))

p_FigS11 <- ggplot(d_trueness, aes(Source, Conc, color = Origin)) +
  geom_point(position = position_jitter(width = .12, height = 0, seed = 1),
             size = 1.3, alpha = .85) +
  geom_errorbar(aes(ymin = Conc - Conc_sd, ymax = Conc + Conc_sd),
                width = .2, na.rm = TRUE) +
  facet_wrap(~ ceramideName, scales = "free_y", ncol = 4) +
  scale_color_manual(values = c("This study (ILS)" = "#ca0020",
                                "External reference" = "grey40"), name = NULL) +
  labs(x = NULL, y = "Concentration in NIST SRM 1950 (\U003BCmol/L)") +
  theme_classic(base_size = 9) +
...
ggsave(plot = p_FigS11, filename = here("manuscript/output/FigS11_Trueness_SRM1950.png"),
       device = "png", dpi = 600, width = 200, height = 80, units = "mm")

p_FigS11

cat("\n=== SRM 1950 concentrations (uM): SP/MP vs external references ===\n")
print(as.data.frame(d_trueness |>
  group_by(ceramideName, Source) |>
  summarise(Conc = round(mean(Conc), 3), .groups = "drop") |>
  pivot_wider(names_from = Source, values_from = Conc)), row.names = FALSE)
```

...
d_full$Protocol <- forcats::fct_relevel(d_full$Protocol, 
                                        c("SOP", "OTHER"))
d_full$SampleType <- forcats::fct_relevel(d_full$SampleType, 
                                          c("SRM", "hTAG", "DB", "YAA"))
d_full$MassAnalyzerResolution <- forcats::fct_relevel(d_full$MassAnalyzerResolution, 
                                                      c("LowRes", "HighRes"))


# Prepare variable `d_samples` with injection replicates averaged and calculated corresponding replicate CVs ------
...
# Prepare variable `d_assays` with sample replicates averaged by SRM and calculated corresponding intra-lab CVs ------
...
d_assays |> select(LabId, LabId_group, LabNum) |> distinct() |>  write_csv(file = here("manuscript/output/LabId-LabNum_mapping.csv"))

```


# 02. Table 2: Consensus concentrations and CVs of the 4 measured ceramides in the 4 NIST Reference Materials
...
# 03. Suppl Table S1: Concentrations and inter-lab %CVs in all reference materials in the full and outlier-filtered datasets
...
```{r manusript-suppl-table-S1}

tableS1_stats_all_datasets <- get_study_stats(d_assays) |>
  select(
    Matrix = SampleType,
    Ceramide = ceramideName,
    'n (All)' = n,
...
tableS1_stats_all_datasets |> 
  ...
  write_excel_csv(file = here("manuscript/output/Suppl-TableS1_Concentrations-CVs_ALL-vs-Outlierfilt-datasets.csv"))
```

# 04. Suppl Table S2: Concentrations and inter-lab %CV of ceramide species from the two protocol types
...
```{r manusript-suppl-table-S2}

tableS2_stats_part1 <- d_assays_filt_mp |>
...
tableS2_stats_part2 <- d_assays_filt_mp |>
...
write_excel_csv(
  x = tableS2_final,
  file = here("manuscript/output/Suppl-TableS2_Concentrations-CVs_SOP-vs_OTHER-MP.csv")
)
```

# 05. Suppl Dataset: Concentrations of all species in all samples from all labs (SP and MP) with Outlier status (TRUE/FALSE) ------
```{r manusript-suppl-dataset}
...
table_all_data_meanreplicates <- flag_outlier(d_assays, limit_iqr = limit_iqr) |> 
  select(SampleType,
         LabNum,
         LabId,
         ceramideName,
...
# 06. Figure 1: All samples from all labs
...
# 07. Figure 2: Single vs multi-point
...
# 09. Figure 4: Interlaboratory Study Comparisons
...
```{r fig04-plot, fig.height=8, fig.width=12, out.width="100%"}
...
# 10. Figure 5: Normalization with different ISTDs
...
# 12. Suppl Figure S1: External Calibration Curves
...
# 13. Suppl Figure S2: Single Point Quantification of NIST SRM 1950
...
# 14. Suppl Figure S3: Single vs multi-point calibration with and without outliers
...
# 15. Suppl Figure S4: SRM comparisons
...
# 16. Suppl Figure S5: Interlaboratory Study Comparisons
...
# 17. Suppl Figure S6: Normalization with different ISTDs (full and outlier-filtered datasets)
...
write_csv(x = d_plot_allistd_mp_stats, here("manuscript/output/FigS06_data-summary-stats.csv"))

```

# 18. Suppl Figure S7: Normalization with different ISTDs (single- vs multi-point calibration, full dataset)
...
write_csv(x = d_plot_allistd_mp_stats, here("manuscript/output/FigS07_data-summary-stats.csv"))

```

# 19. Suppl Figure S8: Normalization to NIST SRM 1950 for hTAG
...
# 20. Suppl Figure S9: Normalization to NIST SRM 1950 for DB
...
# 21. Suppl Figure S10: Normalization to NIST SRM 1950 for YAA
...
d_assays_mstype <- d_assays
d_assays_mstype$MassAnalyzerResolution <- factor(d_assays_mstype$MassAnalyzerResolution)

d_assays_mstype <- flag_outlier(d_assays_mstype, limit_iqr = limit_iqr) |> filter(!Outlier_mp)
d_assays_mstype$SampleType <- forcats::fct_relevel(d_assays_mstype$SampleType, c("SRM","hTAG","DB","YAA")) 

d_assays_mstype_stats_part1 <- d_assays_mstype |> 
  group_by(ceramideName, SampleType) |> 
  nest() |> 
...
d_assays_mstype_stats_part2 <- d_assays_mstype |> 
  group_by(SampleType, ceramideName) |>
  summarise(
...
d_assays_mstype_stats <- d_assays_mstype_stats_part2 |>  left_join(d_assays_mstype_stats_part1)
write_excel_csv(d_assays_mstype_stats, here("manuscript/output/Unpublished_Comparison_LowRes-vs-HighRes-MS_MultiPoint.csv"))
...
summary_tableS2B_sp_stats_part1 <- d_assays_filt_sp |>
  group_by(ceramideName, SampleType) |>
  nest() |>
  mutate(res = purrr::map(data, \(x) broom::tidy(
    t.test(
...
summary_tableS2B_sp_stats_part2 <- d_assays_filt_sp |>
  group_by(SampleType, ceramideName) |>
  summarise(
